...
{
	var projects  = ironWorker(token);
	var project   = projects(proj_id);

	t.ok(project);
	t.ok(typeof project.tasks === 'function');
	t.ok(typeof project.tasks.list === 'function');
	t.ok(project === project.tasks);
	t.ok(project.list === project.tasks.list);
